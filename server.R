  ...
  observeEvent(input$execute,{
    ...
    if (!is.null(input$factura)) {
      ...
      names(facturas)[names(facturas) == 'ï..NÃºmero.de.factura'] <<-
        'Factura'
      names(facturas)[names(facturas) == 'Proveedor'] <<- 'Proveedor'
      names(facturas)[names(facturas) == 'Mes.de.facturaciÃ³n'] <<- 'Mes'
      
      #Convert the ITEM months to YYYY/MM/01
      for (i in 1:12) {
          facturas['Mes'] <<- lapply(facturas['Mes'], function(x) gsub(names(y[i]), y[[i]], x))
      }
      facturas['Fecha'] <<- lapply(facturas['Mes'], function(x) paste(substr(x , 3 , 6),
                                                                     substr(x , 1 , 2),
                                                                     "01",
                                                                     sep = "/"))
    ...
    if (!is.null(input$usos)) {
      #Clear and create variable and read CSV file
        ...
        if (!is.null(input$usosPEN)) {
          dataFilesPEN <<- NULL
          dataFilesPEN <<- lapply(input$usosPEN[['datapath']],read.csv2)
          usoPEN <<- rbindlist(dataFilesPEN)
        }
        if (!is.null(input$usosCOP)) {
          dataFilesCOP <<- NULL
          dataFilesCOP <<- lapply(input$usosCOP[['datapath']],read.csv2)
      ...
      if (client == "pa") {
        ...
        if (client != "hdc" & client != "AguasAndinas" & client != "LMovil") {
          names(uso)[names(uso) == 'N.Â..Copias'] <<-
            'N Copias'
          names(uso)[names(uso) == 'N.Â..Copias.B.N'] <<-
            'N Copias B/N'
          ...
          names(uso)[names(uso) == 'Servicios...Otros..CLP.'] <<-  'Servicios (CLP)'
          ...
          names(uso)[names(uso) == 'Voz.hacia.Int..CLP.'] <<-  'Voz hacia Int (CLP)'
          names(uso)[names(uso) == 'N.Â..Datos.nac.'] <<- 'N. Datos nac.'
          names(uso)[names(uso) == 'Datos.espec...CLP.'] <<- 'Datos espec. (CLP)'
          names(uso)[names(uso) == 'SMS..CLP.'] <<- 'SMS (CLP)'
          names(uso)[names(uso) == 'SMS.nac...CLP.'] <<- 'SMS nac. (CLP)'
          names(uso)[names(uso) == 'SMS.inter...CLP.'] <<- 'SMS inter. (CLP)'
          names(uso)[names(uso) == 'SMS.espec...CLP.'] <<- 'SMS espec. (CLP)'
          names(uso)[names(uso) == 'N.Â..Voz.nacional'] <<- 'N. Voz nacional'
          names(uso)[names(uso) == 'N.Â..Voz.inter.'] <<- 'N. Voz inter.'
          names(uso)[names(uso) == 'N.Â..Voz.hacia.Int'] <<- 'N. Voz hacia Int'
          names(uso)[names(uso) == 'N.Â..Voz.roaming'] <<- 'N. Voz roaming'
          names(uso)[names(uso) == 'N.Â..Datos.inter.'] <<- 'N. Datos inter.'
          names(uso)[names(uso) == 'N.Â..Voz.Roaming.In'] <<- 'N. Voz Roaming In'
          names(uso)[names(uso) == 'N.Â..Voz.Roaming.Out'] <<- 'N. Voz Roaming Out'
          names(uso)[names(uso) == 'N.Â..SMS.nac.'] <<- 'N. SMS nac.'
          names(uso)[names(uso) == 'N.Â..SMS.inter.'] <<- 'N. SMS inter.'
          names(uso)[names(uso) == 'Voz.roaming..CLP.'] <<- 'Voz roaming (CLP)'
          names(uso)[names(uso) == 'Servicios...Opciones..CLP.'] <<- 'Opciones (CLP)'
          
          #Create a column with the phone number without the 56 (Chile)
          uso[,'Acceso fix'] <<- lapply(uso[,'Acceso'], function(x) substring(x, 3))
        }
        
        #If the client got printer access eliminate the thousand dot separator
      if (client != "hdc" & client != "AguasAndinas" & client != "lmovil") {
        uso[,c('N Copias','N Copias B/N','N Copias Color')] <<-
          lapply(uso[,c('N Copias','N Copias B/N','N Copias Color')], function(x) as.character(gsub("\\.", "", x)))
      }
        
        #Convert the ITEM months to YYYY/MM/01
        for (k in 1:12) {
          uso[,'Periodo de'] <<- lapply(uso[,'Periodo de'], function(x) gsub(names(y[k]), y[[k]], x))
        }
        
        uso[,'Fecha'] <<- lapply(uso[,'Periodo de'], function(x) paste(substr(x , 3 , 6),
                                                                      substr(x , 1 , 2),
                                                                      "01",
                                                                      sep = "/"))
      #Delete the Column MES that its not needed now
      ...
      if (client == "pa") {
      ...
      else if (client != "hdc" & client != "AguasAndinas" & client != "lmovil") {
        ...
        uso <<- subset(uso,select = c("Acceso","Proveedor","Total (CLP)","Plano tarifario (CLP)","Uso (CLP)","Servicios (CLP)","Descuentos (CLP)",
                      "Voz (CLP)","Voz Nacional (CLP)", "Voz Inter. (CLP)","Datos (CLP)","Datos espec. (CLP)","Datos Nacional (CLP)",
                      "Voz hacia Int (CLP)","Voz roaming (CLP)","SMS (CLP)", "SMS nac. (CLP)", "SMS inter. (CLP)", "Datos Inter. (CLP)",
                      "SMS espec. (CLP)","Opciones (CLP)","N. Voz nacional","N. Voz inter.","N. Voz hacia Int","N. Voz roaming",
                      "N. Datos nac.","N. Datos inter.","N. Voz Roaming In","N. Voz Roaming Out","N. SMS nac.","N. SMS inter.","Fecha","Acceso fix"))
    ...
    if (!is.null(export)) {
      ...
      file.copy(export$datapath, paste(export$datapath, ".xlsx", sep = ""))
      ...
      if (client == "Subsole") {
        ...
        ACCESSES <- read.table(file = "ACCESSES.txt", encoding = "utf8")
        names(ACCESSES) <-
          c(
            "Acceso",
            "Tipo",
        ...
        ACCESSES <- subset(ACCESSES,select = c("ACCESS.NUMBER","CARRIER_ORG:1","CARRIER_ORG:2","CARRIER_ORG:3"))
        write.table(ACCESSES, file = "ACCESSES.txt", fileEncoding = "utf8")
        ACCESSES <- read.table(file = "ACCESSES.txt", encoding = "utf8")
        names(ACCESSES) <-
          c(
        ...
        ACCESSES[,'Acceso fix'] <- lapply(ACCESSES['Acceso'], function(x) substring(x, 3))
      ...
      if (client == "Subsole") {
        ...
        DEVICES <- read.table(file = "DEVICES.txt", encoding = "utf8")
        names(DEVICES) <-
          c("Tipo",
            "Modelo",
            "REFNUM",
      ...
      write.table(ASSOCIATIONS, file = "ASSOCIATIONS.txt", fileEncoding = "utf8")
      ASSOCIATIONS <-
    ...
    if (!is.null(presupuesto)) {
      ...
      presupuesto <- read.xlsx(presupuesto$datapath,
                         ...
                         startRow = 1)
      names(presupuesto) <-
        c("Gestion","Sociedad","CECO","Tipo de Servicio","Cuenta Contable","Item Servicio","Proveedor","Month","Presupuesto Mensual","Presupuesto Anual")
      file.remove("presupuesto.txt")
      write.table(presupuesto, file = "presupuesto.txt", fileEncoding = "utf8")
      presupuesto <- read.table(file = "presupuesto.txt", encoding = "utf8")
      names(presupuesto) <-
        c("Gestion","Sociedad","CECO","Tipo de Servicio","Cuenta Contable","Item Servicio","Proveedor","Month","Presupuesto Mensual","Presupuesto Anual")
    ...
    if (!is.null(planes)) {
      ...
      for (i in 1:12) {
        PLAN['Mes'] <- lapply(PLAN['Mes'], function(x) gsub(names(y[i]), y[[i]], x))
      }
      
      PLAN['Fecha'] <- lapply(PLAN['Mes'], function(x) paste(substr(x , 3 , 6),
                                                                      substr(x , 1 , 2),
                                                                      "01",
                                                                      sep = "/"))
      ...
      PLAN[,'Acceso fix'] <- lapply(PLAN['Acceso'], function(x) substring(x, 3))
      dbWriteTable(
        DB,
        "plan",
        PLAN,
    ...
    if (!is.null(input$cdr)) {
      ...
      names(cdr)[names(cdr) == 'Red.recurrente'] <<- 'Red recurrente'
      names(cdr)[names(cdr) == 'Red.destinada'] <<- 'Red destinada'
      names(cdr)[names(cdr) == 'Organización.de.gestiòn'] <<- 'Organización de gestion'
      ...
      cdr[,'Numero de llamada fix'] <<- lapply(cdr[,'Numero de llamada'], function(x) substring(x, 3))
      ...
      dbWriteTable(
        ...
        allow.keywords = FALSE
		 ...
		 dbSendQuery(DB,"DROP TABLE IF EXISTS `cdr_join_accesses_all`;")
		  ...
		  dbSendQuery(DB,"ALTER TABLE `cdr` ADD id int NOT NULL AUTO_INCREMENT primary key;")
		  dbSendQuery(DB,"ALTER TABLE `cdr` ADD INDEX `Numero llamado` (`Numero llamado`);")
		  dbSendQuery(DB,"CREATE TABLE cdr_join_accesses_all SELECT * FROM `cdr` LEFT JOIN `accesses` ON `cdr`.`Numero llamado`=`accesses`.`Acceso fix`;")
		  dbSendQuery(DB,"ALTER TABLE `cdr_join_accesses_all` CHANGE COLUMN `Acceso` `Acceso llamado` VARCHAR(255);")
		  dbSendQuery(DB,"ALTER TABLE `cdr_join_accesses_all` CHANGE COLUMN `Proveedor` `Proveedor llamado` VARCHAR(255);")
